        ...
        for ( int i = 0; i < phdr_entries; i++) {
            ...
            for (int j = 0; j < mem_pages; j++) {
                ...
                if ( !is_address_aligned((size_t) hhdm_get_phys_address(offset_address), PAGE_SIZE_IN_BYTES)) {
                    ...
                    void *new_virt_addr = hhdm_get_variable((uintptr_t)phys_address);
                    memcpy(new_virt_addr, (void *)offset_address, phdr.p_memsz);
                    map_phys_to_virt_addr_hh(phys_address, (void *)vaddr_address, VMM_FLAGS_USER_LEVEL | vmm_hdr_flags | VMM_FLAGS_PRESENT, (uint64_t *) new_task->vmm_data.root_table_hhdm);
                } else {
