...
void init_ioapic(MADT *madt_table){
    ...
    if(item != NULL) {
        ...
        if (return_value != 0) {
            pretty_log(Fatal, "PANIC!!");
        }
        io_apic_max_redirections = (uint8_t) (ioapic_version >> 16);
        pretty_logf(Verbose, "Max redirection entries value: 0x%x ", io_apic_max_redirections);
        io_apic_source_override_array_size = parse_io_apic_interrupt_source_overrides(madt_table);
        pretty_logf(Verbose, "Found %x source override entries", io_apic_source_override_array_size);
    }
}
...
int parse_io_apic_interrupt_source_overrides(MADT* table) {
    ...
    pretty_logf(Verbose, "Table Lenght: %x", table->header.Length);
    while(total_length < table->header.Length && source_override_counter < IO_APIC_SOURCE_OVERRIDE_MAX_ITEMS) {
        ...
        if(item != NULL && item->type == MADT_IO_APIC_INTERRUPT_SOURCE_OVERRIDE){
            ...
            pretty_logf(Verbose, "SO Item: type: %x  bus_src: %x - irq_src: %x - GSI: %x -", item->type, so_item->bus_source, so_item->irq_source, so_item->global_system_interrupt);
...
int write_io_apic_redirect(uint8_t index, io_apic_redirect_entry_t redtbl_entry) {
    ...
    pretty_logf(Verbose, "Redirect entry: %x", index);
...
void set_irq(uint8_t irq_type, uint8_t redirect_table_pos, uint8_t idt_entry, uint8_t destination_field, uint32_t flags, bool masked) {
    ...
    while(counter < io_apic_source_override_array_size) {
        ...
        if(io_apic_source_overrides[counter].irq_source == irq_type) {
            ...
            pretty_logf(Verbose, "\t---Source Override found for type: %x using apic pin: %x ", irq_type, selected_pin);
    ...
    pretty_logf(Info, "Setting IRQ number: %x, to idt_entry: %x at REDTBL pos: %x - Final value: %x", irq_type, idt_entry, redirect_table_pos, entry.raw);
    ...
    pretty_logf(Verbose, "ret_val: %d (from read_io_apic_redirect) - entry raw: %x mask: %d", ret_val, read_entry.vector, read_entry.interrupt_mask);
}
...
int set_irq_mask(uint8_t redirect_table_pos, bool masked_status) {
    ...
    if(ret_val == 0) {
        ...
        pretty_log(Verbose, "Invalid redirection table position");
        return -1;
    }
    return 0;
}
