...
void init_ioapic(MADT *madt_table){
    ...
    if(item != NULL) {
        loglinef(Verbose, "IOAPIC Item type: 0x%x", item->type);
        loglinef(Verbose, "IOAPIC Item address: 0x%x - length: 0x%x", item, item->length);
        ...
        loglinef(Verbose, "IOAPIC_ID: 0x%x, Address: 0x%x", ioapic_item->ioapic_id, ioapic_item->address ); 
        loglinef(Verbose, "IOApic_Global_System_Interrupt_Base: 0x%x", ioapic_item->global_system_interrupt_base);
        loglinef(Verbose, "IOApic Higher Half Address: 0x%x", ensure_address_in_higher_half(ioapic_item->address));
        ...
        loglinef(Verbose, "IOAPIC Version: 0x%x", ioapic_version);
        io_apic_redirect_entry_t redtbl_entry;
        int return_value = read_io_apic_redirect(0x10, &redtbl_entry);
        if (return_value != 0) {
            logline(Verbose, "PANIC!!");
        }
        io_apic_max_redirections = (uint8_t) (ioapic_version >> 16);
        loglinef(Verbose, "Max redirection entries value: 0x%x ", io_apic_max_redirections);
        io_apic_source_override_array_size = parse_io_apic_interrupt_source_overrides(madt_table);
        loglinef(Verbose, "Found %x source override entries", io_apic_source_override_array_size);
        loglinef(Verbose, "---- SO Item: bus_source: %x - irq_source: %x - GSI: %x", io_apic_source_overrides[1].bus_source, io_apic_source_overrides[1].irq_source, io_apic_source_overrides[1].global_system_interrupt);
    }
}

int parse_io_apic_interrupt_source_overrides(MADT* table) {
    loglinef(Verbose, "madt_address: 0x%x", (uint64_t) table);
    ...
    loglinef(Verbose, "Table Lenght: %x", table->header.Length);
    while(total_length < table->header.Length && source_override_counter < IO_APIC_SOURCE_OVERRIDE_MAX_ITEMS) {
        ...
        if(item != NULL && item->type == MADT_IO_APIC_INTERRUPT_SOURCE_OVERRIDE){
            ...
            loglinef(Verbose, "--SO Item: type: %x  bus_src: %x - irq_src: %x - GSI: %x -", item->type, so_item->bus_source, so_item->irq_source, so_item->global_system_interrupt);
...
int write_io_apic_redirect(uint8_t index, io_apic_redirect_entry_t redtbl_entry) {
    ...
    loglinef(Verbose, "Setting redirect entry: %x", index);
...
void set_irq(uint8_t irq_type, uint8_t redirect_table_pos, uint8_t idt_entry, uint8_t destination_field, uint32_t flags, bool masked) {
    ...
    while(counter < io_apic_source_override_array_size) {
        ...
        if(io_apic_source_overrides[counter].irq_source == irq_type) {
            ...
            loglinef(Verbose, "---Source Override found for type: %x using apic pin: %x ", irq_type, selected_pin);
    ...
    loglinef(Verbose, "Setting IRQ number: %x, to idt_entry: %x at REDTBL pos: %x - Final value: %x", irq_type, idt_entry, redirect_table_pos, entry.raw);
    ...
    loglinef(Verbose, "ret_val: %d - entry raw: %x mask: %d", ret_val, read_entry.vector, read_entry.interrupt_mask);
}
...
int set_irq_mask(uint8_t redirect_table_pos, bool masked_status) {
    ...
    if(ret_val == 0) {
        ...
        logline(Verbose, "Invalid redirection table position");
        return -1;
    }
    return 0;
}
