...
void parse_RSDTv2(RSDPDescriptor20 *descriptor){
    loglinef(Verbose, "(parse_RSDT) Parse RSDP v2 Descriptor\n");
    loglinef(Verbose, "(parse_RSDT) - Descriptor physical address: 0x%x", ALIGN_PHYSADDRESS(descriptor->XsdtAddress));
    ...
    loglinef(Verbose, "(parse_RSDT) - XSDT_Length: 0x%x", descriptor->Length);
    ACPISDTHeader header = xsdt_root->header;
    loglinef(Verbose, "(parse_RSDT) - XSDT_Signature: %.4s", header.Signature);
    sdt_version = RSDT_V2;

    size_t required_extra_pages = (header.Length / KERNEL_PAGE_SIZE) + 1;

    if (required_extra_pages > 1) {
        loglinef(Verbose, "(parse_RSDT) - XSDT_PAGES_NEEDED: %d", required_extra_pages);
        for (size_t j = 1; j < required_extra_pages; j++) {
    ...
    loglinef(Verbose, "(parse_RSDT) - Total xsdt Tables: %d", rsdtTablesTotal);
